...
def rotation_matrix_to_axis_angle(r: torch.Tensor):
    ...
    cos_angle = (r.diagonal(dim1=1, dim2=2).sum(dim=1) - 1) / 2
    cos_angle = torch.clamp(cos_angle, -1.0, 1.0)
    angle = torch.acos(cos_angle)
    
    # Special case: angle is 0
    sin_angle = torch.sin(angle)
    mask_0 = torch.abs(sin_angle) < 1e-6
    
    # Calculate rotation axis
    axis = torch.zeros((batch_size, 3), device=r.device)
    
    # When angle is approximately 0 (the rotation matrix is approximately identity matrix),
    # any axis can be used. We use the z-axis as default.
    axis[mask_0] = torch.tensor([0.0, 0.0, 1.0], device=r.device)
    
    # When angle is not approximately 0
    mask_not_0 = ~mask_0
    if mask_not_0.any():
        # Calculate rotation axis (r21-r12, r02-r20, r10-r01)
        axis_not_0 = torch.zeros((mask_not_0.sum(), 3), device=r.device)
        r_not_0 = r[mask_not_0]
        
        axis_not_0[:, 0] = r_not_0[:, 2, 1] - r_not_0[:, 1, 2]
        axis_not_0[:, 1] = r_not_0[:, 0, 2] - r_not_0[:, 2, 0]
        axis_not_0[:, 2] = r_not_0[:, 1, 0] - r_not_0[:, 0, 1]
        
        # When angle is approximately π (the rotation matrix is approximately -identity matrix),
        mask_pi = (cos_angle[mask_not_0] < -0.99)
        if mask_pi.any():
            r_pi = r_not_0[mask_pi]
            diag = r_pi.diagonal(dim1=1, dim2=2)
            max_indices = torch.argmax(diag, dim=1)
            
            for i, idx in enumerate(max_indices):
                if idx == 0:  # x axis
                    axis_not_0[mask_pi][i, 0] = torch.sqrt((r_pi[i, 0, 0] + 1) / 2)
                    axis_not_0[mask_pi][i, 1] = r_pi[i, 0, 1] / (2 * axis_not_0[mask_pi][i, 0])
                    axis_not_0[mask_pi][i, 2] = r_pi[i, 0, 2] / (2 * axis_not_0[mask_pi][i, 0])
                elif idx == 1:  # y axis
                    axis_not_0[mask_pi][i, 1] = torch.sqrt((r_pi[i, 1, 1] + 1) / 2)
                    axis_not_0[mask_pi][i, 0] = r_pi[i, 0, 1] / (2 * axis_not_0[mask_pi][i, 1])
                    axis_not_0[mask_pi][i, 2] = r_pi[i, 1, 2] / (2 * axis_not_0[mask_pi][i, 1])
                else:  # z axis
                    axis_not_0[mask_pi][i, 2] = torch.sqrt((r_pi[i, 2, 2] + 1) / 2)
                    axis_not_0[mask_pi][i, 0] = r_pi[i, 0, 2] / (2 * axis_not_0[mask_pi][i, 2])
                    axis_not_0[mask_pi][i, 1] = r_pi[i, 1, 2] / (2 * axis_not_0[mask_pi][i, 2])
        
        # Normalize rotation axis
        norm = torch.norm(axis_not_0, dim=1, keepdim=True)
        axis_not_0 = axis_not_0 / norm
        
        axis[mask_not_0] = axis_not_0
    
    # Combine angle and axis to get axis-angle representation
    axis_angle = axis * angle.view(-1, 1)
